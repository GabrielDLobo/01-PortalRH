...
class TerminationRequest(models.Model):
    ...
    motivo = models.ForeignKey(
        TerminationReason, on_delete=models.CASCADE, verbose_name="Motivo do Desligamento"
    )

    # Termination Details
    data_ultimo_dia = models.DateField(verbose_name="Último Dia de Trabalho")
    data_desligamento = models.DateField(
        help_text="Data oficial do desligamento", verbose_name="Data de Desligamento"
    )
    ...
    data_aprovacao_rh = models.DateTimeField(
        null=True, blank=True, verbose_name="Data de Aprovação RH"
    )

    comentario_aprovacao_rh = models.TextField(
        blank=True, verbose_name="Comentário da Aprovação RH"
    )
    ...
    def __str__(self) -> str:
        return (
            f"Desligamento: {self.funcionario} - {self.motivo.nome} ({self.get_status_display()})"
        )
...
class TerminationDocument(models.Model):
    ...
    tipo_documento = models.CharField(
        max_length=20, choices=DocumentTypeChoices.choices, verbose_name="Tipo de Documento"
    )
