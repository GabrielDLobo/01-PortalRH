...
class TerminationReasonAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações Básicas", {"fields": ("codigo", "nome", "descricao", "ativo")}),
...
class TerminationRequestAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "motivo", "created_at"]
    search_fields = [
    ...
    ordering = ["-created_at"]
    readonly_fields = ["created_at", "updated_at", "aprovador_rh", "data_aprovacao_rh"]
    ...
    fieldsets = (
        ...
        ("Documentos", {"fields": ("anexo_documentos",)}),
        ("Timestamps", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
