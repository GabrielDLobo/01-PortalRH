...
class TerminationRequestViewSet(viewsets.ModelViewSet):
    ...
    def reject(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"message": "Solicitação rejeitada.", "status": termination_request.status}
            )
    ...
    def start_processing(self, request, pk=None):
        ...
        return Response(
            {"message": "Processamento da rescisão iniciado.", "status": termination_request.status}
        )
    ...
    def complete(self, request, pk=None):
        ...
        return Response(
            {"message": "Processo de desligamento concluído.", "status": termination_request.status}
        )
    ...
    def cancel(self, request, pk=None):
        ...
        if request.user != termination_request.solicitante and not (
            hasattr(request.user, "is_admin_rh") and request.user.is_admin_rh
        ):
            return Response(
        ...
        return Response({"message": "Solicitação cancelada.", "status": termination_request.status})
    ...
    def stats(self, request):
        ...
        stats = {
            ...
            "pendente_rh": queryset.filter(
                status=TerminationRequest.StatusChoices.PENDENTE_RH
            ).count(),
            "aprovada_rh": queryset.filter(
                status=TerminationRequest.StatusChoices.APROVADA_RH
            ).count(),
            "rejeitada_rh": queryset.filter(
                status=TerminationRequest.StatusChoices.REJEITADA_RH
            ).count(),
            "processando": queryset.filter(
                status=TerminationRequest.StatusChoices.PROCESSANDO
            ).count(),
        ...
        for i in range(12):
            ...
            por_mes.append({"mes": month_start.strftime("%Y-%m"), "count": count})
