...
class PreAdmissionRH(models.Model):
    ...
    weekly_workload = models.CharField(
        max_length=10, choices=WORKLOAD_CHOICES, verbose_name="Jornada Semanal"
    )
    contract_type = models.CharField(
        max_length=20, choices=CONTRACT_TYPE_CHOICES, verbose_name="Tipo de Contrato"
    )
    salary = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Salário")
    benefits = models.TextField(
        verbose_name="Benefícios (vale-transporte, refeição, plano de saúde, etc.)", blank=True
    )
    start_date = models.DateField(verbose_name="Data de Início")
    vacation_policy = models.TextField(
        verbose_name="Política de Férias e Banco de Horas", blank=True
    )
    ...
    employee_user_created = models.BooleanField(
        default=False, verbose_name="Usuário do funcionário criado"
    )
    temporary_password = models.CharField(
        max_length=50, blank=True, verbose_name="Senha temporária"
    )
    email_sent = models.BooleanField(default=False, verbose_name="E-mail enviado")
    employee = models.OneToOneField(
        "Employee", on_delete=models.CASCADE, null=True, blank=True, related_name="pre_admission_rh"
    )
...
class Employee(models.Model):
    ...
    full_name = models.CharField(
        max_length=255, verbose_name="Nome Completo", blank=True, null=True
    )
    ...
    # Work Documents
    pis_pasep = models.CharField(max_length=15, verbose_name="PIS/PASEP", blank=True, null=True)
    work_card_number = models.CharField(
        max_length=20, verbose_name="Número da CTPS", blank=True, null=True
    )
    work_card_series = models.CharField(
        max_length=10, verbose_name="Série da CTPS", blank=True, null=True
    )
    ...
    bank_name = models.CharField(
        max_length=100, verbose_name="Nome do Banco", blank=True, null=True
    )
    bank_code = models.CharField(
        max_length=6, verbose_name="Código do Banco", blank=True, null=True
    )
    ...
    requires_password_change = models.BooleanField(
        default=False, verbose_name="Requer Troca de Senha"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.employee_id:
            ...
            if last_employee:
                last_id = (
                    int(last_employee.employee_id.split("-")[1])
                    if "-" in last_employee.employee_id
                    else 0
                )
...
class EmployeeDocument(models.Model):
    ...
    document_type = models.CharField(
        max_length=30, choices=DOCUMENT_TYPE_CHOICES, verbose_name="Tipo de Documento"
    )
...
class AdmissionProcess(models.Model):
    ...
    employee = models.OneToOneField(
        Employee, on_delete=models.CASCADE, related_name="admission_process"
    )
    ...
    personal_info_completed = models.BooleanField(
        default=False, verbose_name="Informações Pessoais Preenchidas"
    )
    ...
    def completion_percentage(self):
        ...
        total_steps = 3
        completed_steps = sum(
            [self.personal_info_completed, self.documents_uploaded, self.hr_review_completed]
        )
