...
class PreAdmissionRHSerializer(serializers.ModelSerializer):
    ...
    weekly_workload_display = serializers.CharField(
        source="get_weekly_workload_display", read_only=True
    )
    ...
    def validate_personal_email(self, value):
        ...
        if self.instance:
            ...
            if (
                PreAdmissionRH.objects.filter(personal_email=value)
                .exclude(id=self.instance.id)
                .exists()
            ):
                raise serializers.ValidationError(
                    "Este e-mail já está cadastrado em outro pré-cadastro."
                )
        else:
