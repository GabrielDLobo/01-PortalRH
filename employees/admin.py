...
class EmployeeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Escolaridade", {"fields": ("education_level",)}),
        (
        ...
        ("Informações de Trabalho", {"fields": ("department", "position", "hire_date", "salary")}),
...
class AdmissionProcessAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "personal_info_completed", "documents_uploaded", "hr_review_completed"]
...
class PreAdmissionRHAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações Básicas", {"fields": ("personal_email", "full_name")}),
        (
        ...
        for pre_admission in queryset:
            if not pre_admission.employee_user_created:
                try:
                    ...
                    self.message_user(
                        request, f"Erro ao criar conta para {pre_admission.full_name}: {str(e)}"
                    )
