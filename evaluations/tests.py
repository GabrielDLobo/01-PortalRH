...
from .models import Evaluation, EvaluationCriteria, EvaluationTemplate
...
class EvaluationTemplateTestCase(TestCase):
    ...
    def test_template_creation(self):
        ...
        template = EvaluationTemplate.objects.create(
            nome="Avaliação Anual", descricao="Avaliação de desempenho anual", ativo=True
        )
    ...
    def test_template_uniqueness(self):
        ...
        EvaluationTemplate.objects.create(nome="Template Único", ativo=True)
...
class EvaluationCriteriaTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.template = EvaluationTemplate.objects.create(nome="Template Test", ativo=True)
    ...
    def test_criteria_unique_together(self):
        ...
        EvaluationCriteria.objects.create(
            template=self.template, nome="Critério 1", peso=Decimal("1.00")
        )

        with self.assertRaises(Exception):  # IntegrityError
            EvaluationCriteria.objects.create(
                template=self.template, nome="Critério 1", peso=Decimal("1.50")
            )
...
class EvaluationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.template = EvaluationTemplate.objects.create(nome="Annual Evaluation", ativo=True)
...
class EvaluationAuthorizationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.evaluator = User.objects.create_user(
            username="evalauth_manager@test.com",
            email="evalauth_manager@test.com",
            password="testpass123",
            role="funcionario",
        )
        self.evaluated = User.objects.create_user(
            username="evalauth_employee@test.com",
            email="evalauth_employee@test.com",
            password="testpass123",
            role="funcionario",
        )
        self.uninvolved_user = User.objects.create_user(
            username="evalauth_outsider@test.com",
            email="evalauth_outsider@test.com",
            password="testpass123",
            role="funcionario",
        )
