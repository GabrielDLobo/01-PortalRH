...
from app.permissions import CanManageEvaluation, CanViewEvaluation, IsAdminRH, IsStaffOrAdminRH
...
class EvaluationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_admin_rh:
            ...
            return self.queryset.filter(Q(avaliado=user) | Q(avaliador=user))
    ...
    def stats(self, request):
        ...
        # Average score
        avg_score = evaluations.filter(nota_final__isnull=False).aggregate(Avg("nota_final"))[
            "nota_final__avg"
        ]
        ...
        evaluations_by_type = dict(
            evaluations.values("tipo").annotate(count=Count("id")).values_list("tipo", "count")
        )
        ...
        top_performers = (
            evaluations.filter(nota_final__isnull=False)
            .values("avaliado__first_name", "avaliado__last_name")
            .annotate(avg_score=Avg("nota_final"))
        ...
        improvement_needed = (
            evaluations.filter(nota_final__lt=5.0)
            .values("avaliado__first_name", "avaliado__last_name")
            .annotate(avg_score=Avg("nota_final"))
            .order_by("avg_score")[:5]
        )
    ...
    def get_queryset(self):
        ...
        if user.is_admin_rh:
            ...
            return self.queryset.filter(
                models.Q(avaliacao__avaliador=user) | models.Q(avaliacao__avaliado=user)
            )
...
class EvaluationCycleViewSet(viewsets.ModelViewSet):
    ...
    def start(self, request, pk=None):
        ...
        return Response(
            {"message": "Ciclo de avaliação iniciado com sucesso.", "status": cycle.status}
        )
    ...
    def complete(self, request, pk=None):
        ...
        return Response(
            {"message": "Ciclo de avaliação finalizado com sucesso.", "status": cycle.status}
        )
