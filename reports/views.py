...
from .serializers import (
    ...
    ReportTemplateCreateUpdateSerializer,
    ReportTemplateDetailSerializer,
    ReportTemplateListSerializer,
)
from .services import CacheService, ExportService, ReportService
...
class ReportTemplateViewSet(viewsets.ModelViewSet):
    ...
    def execute(self, request, pk=None):
        ...
        if execution_serializer.is_valid():
            ...
            try:
                ...
                report_service.execute_report(execution)
    ...
    def bookmark(self, request, pk=None):
        ...
        bookmark_serializer = ReportBookmarkSerializer(
            data=bookmark_data, context={"request": request}
        )
...
class ReportExecutionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def download(self, request, pk=None):
        ...
        # Check permissions
        if execution.executed_by != request.user and request.user.role not in ["admin", "rh"]:
            return Response(
        ...
        if not execution.file_path:
            return Response(
                {"error": "No file available for download"}, status=status.HTTP_404_NOT_FOUND
            )
...
class ReportScheduleViewSet(viewsets.ModelViewSet):
    ...
    def pause(self, request, pk=None):
        ...
        return Response(
            {"error": "Schedule is not in active status"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def resume(self, request, pk=None):
        ...
        return Response({"error": "Schedule is not paused"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def summary(self, request):
        ...
        try:
            ...
            summary_data = {
                ...
                "pending_leave_requests": LeaveRequest.objects.filter(status="pendente").count(),
                "pending_evaluations": PerformanceEvaluation.objects.filter(status="draft").count(),
            }
            ...
            term_data = (
                TerminationRequest.objects.filter(created_at__gte=six_months_ago)
            ...
            summary_data["terminations_by_month"] = [
                {"month": item["month"], "count": item["count"]} for item in term_data
            ]

            # Leave requests by status
            leave_data = LeaveRequest.objects.values("status").annotate(count=Count("id"))

            summary_data["leave_requests_by_status"] = [
                {"status": item["status"], "count": item["count"]} for item in leave_data
            ]
        ...
        try:
            ...
            if output_format == "pdf":
                file_content = export_service.export_to_pdf(
                    data, report_type, include_charts=export_data.get("include_charts", False)
                )
