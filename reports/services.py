...
class ReportService:
    ...
    def execute_report(self, execution: ReportExecution) -> Dict[str, Any]:
        ...
        try:
            ...
            data = self.generate_report(
                execution.template.report_type, filters, execution.executed_by
            )

            # Save results
            execution.complete_execution(result_data=data, rows_processed=len(data.get("data", [])))
                ...
                employees_data.append(
                    {
                        ...
                        "phone": emp.phone,
                        "education_level": emp.get_education_level_display()
    ...
    def _generate_terminations_report(self, filters: Dict[str, Any], user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            # Reason breakdown
            reason_breakdown = (
                queryset.values("motivo__nome").annotate(count=Count("id")).order_by("-count")
            )
            ...
            for i in range(12):
                ...
                count = queryset.filter(
                    data_desligamento__gte=month_start, data_desligamento__lt=month_end
                ).count()
                monthly_breakdown.append({"month": month_start.strftime("%Y-%m"), "count": count})
    ...
    def _generate_admissions_report(self, filters: Dict[str, Any], user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            queryset = Employee.objects.filter(admission_process__isnull=False).select_related(
                "admission_process"
            )
                ...
                admissions_data.append(
                    {
                        ...
                        "process_completed": process.completed_at.isoformat()
            ...
            # Status breakdown
            status_breakdown = AdmissionProcess.objects.values("status").annotate(count=Count("id"))

            # Completion stages
            completion_stats = {
                "personal_info_completed": AdmissionProcess.objects.filter(
                    personal_info_completed=True
                ).count(),
                "documents_uploaded": AdmissionProcess.objects.filter(
                    documents_uploaded=True
                ).count(),
                "hr_review_completed": AdmissionProcess.objects.filter(
        ...
        if "data" in data and data["data"]:
            ...
            if records:
                ...
                for col, header in enumerate(headers, 1):
                    cell = ws.cell(
                        row=current_row, column=col, value=header.replace("_", " ").title()
                    )
        ...
        for column in ws.columns:
            ...
            for cell in column:
                try:
                    ...
                    # Best-effort column sizing; non-numeric cell values are expected here.
                    pass
