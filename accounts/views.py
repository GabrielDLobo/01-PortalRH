...
from app.permissions import CanUpdateOwnProfile, IsAdminRH
...
def first_login_password_change(request):
    ...
    if not employee.requires_password_change:
        return Response(
            {"error": "Usuário não precisa trocar a senha."}, status=status.HTTP_400_BAD_REQUEST
        )
...
def check_password_change_required(request):
    ...
    return Response({"requires_password_change": requires_password_change})
