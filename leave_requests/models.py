...
class LeaveRequest(models.Model):
    ...
    tipo = models.ForeignKey(
        LeaveType, on_delete=models.CASCADE, verbose_name="Tipo de Solicitação"
    )
    ...
    tem_abono_pecuniario = models.BooleanField(
        default=False, verbose_name="Há Venda do Abono Pecuniário?"
    )
    ...
    anexo = models.FileField(
        upload_to="leave_requests/attachments/", null=True, blank=True, verbose_name="Anexo"
    )
    ...
    def clean(self) -> None:
        ...
        if self.data_inicio and self.data_fim:
            ...
            if self.data_inicio < date.today():
                raise ValidationError({"data_inicio": "Data de início não pode ser no passado."})
        ...
        if self.tipo and ("rias" in self.tipo.nome.lower() or "annual" in self.tipo.nome.lower()):
            ...
            if self.dias_gozo <= 0:
                raise ValidationError({"dias_gozo": "Dias em gozo deve ser maior que zero."})
    ...
    def calcular_data_fim_automatica(self) -> Optional[date]:
        ...
        if not (
            self.tipo and ("rias" in self.tipo.nome.lower() or "annual" in self.tipo.nome.lower())
        ):
            return None
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            self.tipo
            and ("rias" in self.tipo.nome.lower() or "annual" in self.tipo.nome.lower())
            and self.dias_gozo
...
class LeaveBalance(models.Model):
    ...
    def use_days(self, dias: int) -> None:
        if self.can_request_days(dias):
            ...
            raise ValueError(
                f"Saldo insuficiente. Disponível: {self.dias_restantes}, Solicitado: {dias}"
            )
